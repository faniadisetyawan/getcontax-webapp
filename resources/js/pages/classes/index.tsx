...
export default function IndexPage({ metaOptions, responseData }: Props) {
    ...
    const handleDelete = (school_class: SchoolClass) => {
        MySwalTheme.fire({
            title: "Are you sure?",
            html: `This <b>"${school_class.name}"</b> will be permanently deleted.<br/><small class="text-danger">This action cannot be undone.</small>`,
            icon: "question",
            confirmButtonText: "Delete",
            showCancelButton: true,
        }).then((val) => {
            if (val.isConfirmed) {
                return router.delete(route('master.classes.destroy', school_class.id));
            }
        });
    }
    ...
    const columns = [
        columnHelper.display({
            ...
            cell: (info) => (
                <Dropdown drop="end" className="no-caret">
                    ...
                    <Dropdown.Menu>
                        <Dropdown.Item onClick={() => router.visit(route('master.classes.edit', info.row.original.id))} role="button">Edit</Dropdown.Item>
        ...
        columnHelper.accessor('name', { header: 'Nama Kelas' }),
        columnHelper.accessor('level', { header: 'Tingkat' }),
        columnHelper.accessor('homeroom_teacher', {
            header: 'Wali Kelas',
            cell: info => info.getValue()?.name || <span className="text-muted">-</span>,
        }),
        columnHelper.accessor('students_count', {
            header: 'Jumlah Siswa',
            cell: info => `${info.getValue()} siswa`,
        }),
    ];
    ...
    return (
        <AppLayout>
            ...
            <div className="mb-4 d-flex flex-wrap align-items-center justify-content-between gap-3">
                ...
                <div>
                    <Button variant="primary" onClick={() => router.visit(route('master.classes.create'))}>Tambah</Button>
                </div>
            </div>
